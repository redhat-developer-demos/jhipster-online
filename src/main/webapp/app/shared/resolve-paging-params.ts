...
function parseAscending(sort: string): boolean {
...
export class ResolvePagingParams implements Resolve<PagingParams> {
  resolve(route: ActivatedRouteSnapshot): PagingParams {
    ...
    return {
      ...
      predicate: sort.split(',')[0].split('%2C')[0],
      ascending: parseAscending(sort)
    };
  }
}

/** @deprecated Use ResolvePagingParams */
export { ResolvePagingParams as JhiResolvePagingParams };
