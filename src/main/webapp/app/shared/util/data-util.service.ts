...
/** @deprecated Use FileLoadError */
export type JhiFileLoadError = FileLoadError;

/** @deprecated Use FileLoadErrorType */
export type JhiFileLoadErrorType = FileLoadErrorType;
...
export class DataUtilsService {
  ...
  openFile(contentType: string, data: string): void {
    ...
    if (window.navigator && nav.msSaveOrOpenBlob) {
      ...
      if (win) {
        win.document.write('<iframe src="' + fileURL + '" width="100%" height="100%"></iframe>');
      }
    }
  }
  ...
  private formatAsBytes(sizeVal: number): string {
    return sizeVal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ') + ' bytes';
  }
}

/** @deprecated Use DataUtilsService */
export { DataUtilsService as JhiDataUtils };
